...
namespace BotBits.DefaultCommands
{
    internal sealed class EditCommands : CommandsBase<EditCommands>
    {
        ...
        private void GodModeCommand(IInvokeSource source, ParsedRequest request)
        {
            this.RequireEdit();
            ...
            if (request.Count >= 1)
            {
                try
                {
                    enabled = bool.Parse(request.Args[0]);
                }
                catch (Exception ex)
                {
        ...
        private void ModModeCommand(IInvokeSource source, ParsedRequest request)
        {
        ...
        private void AdminModeCommand(IInvokeSource source, ParsedRequest request)
        {
